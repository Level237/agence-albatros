<section x-data="{ 
    ...
    <div class="relative h-full w-full">
        <template x-for="slide in slides" :key="slide.id">
            <div x-show="activeSlide === slide.id" x-transition:enter="transition duration-1000 ease-in-out"
                ...
                <div class="absolute inset-0">
                    <img :src="slide.img" class="w-full h-full object-cover transform scale-110 animate-slow-zoom"
                        :alt="slide.title">
                    <!-- Premium Overlays -->
                ...
                <div class="relative h-full container mx-auto px-4 sm:px-6 lg:px-8 flex items-center">
                    <div class="max-w-4xl pt-32 md:pt-20">
                        ...
                        <div x-show="activeSlide === slide.id"
                            ...
                            <a href="#services"
                                class="group relative px-8 py-4 md:px-10 md:py-5 bg-white text-primary text-center font-bold rounded-full overflow-hidden transition-all hover:pr-14 hover:shadow-2xl hover:shadow-white/20 active:scale-95 text-sm md:text-base">
                                <span class="relative z-10" x-text="slide.cta_primary"></span>
                                <svg class="absolute right-6 top-1/2 -translate-y-1/2 w-5 h-5 opacity-0 -translate-x-4 transition-all group-hover:opacity-100 group-hover:translate-x-0 hidden md:block"
                            ...
                            <a href="#contact"
                                class="px-8 py-4 md:px-10 md:py-5 border-2 border-white/20 text-white text-center font-bold rounded-full transition-all hover:bg-white/10 hover:border-white/40 active:scale-95 text-sm md:text-base">
    ...
    <div class="absolute bottom-8 md:bottom-12 left-0 w-full z-30">
        <div
            class="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-0">
            <!-- Indicators -->
            <div class="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start">
                <template x-for="slide in slides" :key="slide.id">
                    <button @click="setSlide(slide.id)"
                        ...
                        <div x-show="activeSlide === slide.id" class="absolute inset-0 bg-white h-full transition-none"
                            :style="'width: ' + progress + '%'"></div>
                    </button>
                </template>
                <div
                    class="ml-4 text-white/50 font-mono text-[10px] md:text-xs tracking-widest uppercase hidden sm:block">
                    <span class="text-white" x-text="'0' + activeSlide"></span> / <span
            ...
            <div class="flex items-center gap-3 md:gap-4">
                <button @click="prev()"
                    class="p-3 md:p-4 rounded-full border border-white/20 text-white transition-all hover:bg-white hover:text-primary backdrop-blur-md active:scale-90 group">
                    <svg class="w-5 h-5 md:w-6 md:h-6 transition-transform group-hover:-translate-x-1" fill="none"
